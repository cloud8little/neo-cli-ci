...
def multithread_run(logger, cfg_request, cfg_response):
    ...
    for i in range(Config.THREAD):
        t = TaskThread(con, args=cfg_request)
        thread_list.append(t)
        t.start()

    for t in thread_list:
        t.join()
        result = result and judge_result(cfg_response, t.get_result())
    ...
    def exec(self, heartbeat_gap=5, message_cb=None):
        ...
        while True:
            try:
                ...
                if self.terminated:
                    raise RuntimeError("thread t2 closed")
